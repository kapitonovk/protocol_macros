...
Private Sub RunSection_1_1(buildBody As Boolean, buildAppendix As Boolean)
    ...
    bodyPath = GetSourcePathByNumber(wbMain, "0.1") ' почему, если под 0.1 лежит темплейт титульника, а боди - 0.2?
    appendixPath = GetSourcePathByNumber(wbMain, "0.2") ' почему, если APPX темплейт под 0.4?
    dataPath = GetSourcePathByNumber(wbMain, "1") ' почему он захардкоден, если мы по ним итерируемся?
