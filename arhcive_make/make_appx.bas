...
' ============================================================
' ГЛАВНАЯ ТОЧКА ВХОДА
' Открывает шаблон приложения, находит data-файл через Sources,
' и последовательно заполняет все таблицы приложения 1.1
' ============================================================
Sub RunAppendixOnly()
    ...
    ' Путь к шаблону приложения (Номер = 0.2)
    For i = 2 To lastRow
    ...
    ' Путь к data-файлу раздела 1 (Номер = 1)
    For i = 2 To lastRow
        numVal = wsSources.Cells(i, 1).Value
...
' ============================================================
' Оркестратор: вызывает заполнение всех 6 таблиц приложения
...
' Переводит букву столбца Excel ("B", "AF") в числовой индекс
...
' Ищет строку в Sources по номеру
...
' Находит номер столбца по заголовку в первой строке листа
...
' Ищет в Mapper значение для скалярного плейсхолдера (например {{АДРЕС}})
' Строки типа SCALAR: колонка 1=Kind, 5=ExcelSheet, 6=ExcelColumnRef, 8=Placeholder
' Строки типа APPX_111b (без Kind): колонка 2=WordTable, 5=ExcelSheet, 6=ExcelColumnRef, 8=Placeholder
...
' Заменяет плейсхолдер во всём документе Word
...
' Безопасно пишет текст в ячейку Word-таблицы.
' Если ячейка не существует из-за merge — просто пропускает,
' вместо падения с "запрашиваемый номер семейства не существует"
Sub SafeSetCellText(wdTable As Object, rowIndex As Long, colIndex As Long, txt As String)
    On Error GoTo SkipCell
    wdTable.Cell(rowIndex, colIndex).Range.Text = txt
    Exit Sub
SkipCell:
End Sub


' Безопасно читает текст ячейки. Возвращает "" если ячейки нет.
...
' Извлекает код категории (например "1К") из текста ячейки вида "{{1К}}"
...
' Добавляет rowsToAdd пустых строк сразу после afterRowIndex,
' НЕ используя Range.Copy (что ломается на вертикальных merge).
...
Sub FillTable_APPX_111b(wdDoc As Object, activeCats As Object)
    ...
    For r = wdTable.Rows.Count To 1 Step -1
        ...
        If catCode <> "" Then
            ...
            Else
                ph = "{{" & catCode & "}}"
                Call ReplacePlaceholderInWord(wdDoc.Application.ActiveDocument, ph, Format(activeCats(catCode), "#,##0"))
            End If
        End If
    Next r
...
' Реальная структура (из анализа docx):
...
'              ячейка 1 = gridSpan 6 (метка), ячейка 2 = gridSpan 2
'              (значение) и содержит плейсхолдер {{1К}} и т.п.
' Данные заливаются с Row 3, подвал — по колонке 7 (после сдвига merge)
' ============================================================
Sub FillTable_APPX_112(wbMain As Workbook, wbData As Workbook, wdDoc As Object, activeCats As Object)
    ...
    Const SAMPLE_ROW As Long = 2   ' контент начинается со строки 2
    ...
    For r = wdTable.Rows.Count To 1 Step -1
        ...
        rowText = wdTable.Rows(r).Range.Text
        On Error GoTo 0
    ...
    ' одна строка уже есть в шаблоне (строка 2),
    ' если данных больше — вставляем новые строки ПЕРЕД тем,
    ' что сейчас находится ниже строки 2
    ' =======================================================
    If rowCount > 1 Then
        For i = 1 To rowCount - 1
            wdTable.Rows.Add BeforeRow:=wdTable.Rows(SAMPLE_ROW + i)
        Next i
    End If
...
' Структура: 16 строк, 6 колонок, БЕЗ merge.
' Row 1 = шапка, Rows 2.. = данные (построчно с 6 показателями)
' WordColumnOrder в Mapper для APPX_113a НЕ заполнен числами —
' поэтому колонка вычисляется по порядку появления строк в Mapper
' (1-я строка блока -> колонка 1, 2-я -> колонка 2, и т.д.)
' ============================================================
Sub FillTable_APPX_113a(wbMain As Workbook, wbData As Workbook, wdDoc As Object)
    ...
    Const SAMPLE_ROW As Long = 2   ' строка 1 = шапка, данные начинаются со строки 2

    Set wsMapper = wbMain.Worksheets("Mapper")
    lastMapRow = wsMapper.Cells(wsMapper.Rows.Count, 2).End(xlUp).Row
    mapFound = False

    ' Берём базовые параметры блока APPX_113a из первой строки блока
    For r = 2 To lastMapRow
    ...
    If lastDataRow < startRow Then
        MsgBox "В листе " & excelSheetName & " нет данных для APPX_113a."
        Exit Sub
    End If
    ...
    ' Если в шаблоне строк меньше, чем нужно под данные, можно либо падать,
    ' либо добавлять строки. Пока делаем проверку.
    If wdTable.Rows.Count < SAMPLE_ROW + rowCount - 1 Then
        MsgBox "В таблице APPX_113a не хватает строк под данные Excel."
        Exit Sub
    End If

    ' Построчно заливаем Excel -> Word
    For i = 0 To rowCount - 1
        wordRow = SAMPLE_ROW + i
...
' поэтому Rows(n) / Rows.Add BeforeRow:=Rows(n) недоступны —
' Word выбрасывает error 5991. Вставка строк идёт через
' Selection.InsertRowsBelow, без обращения к коллекции Rows.
' ============================================================
Sub FillTable_APPX_113b(wbMain As Workbook, wbData As Workbook, wdDoc As Object)
    ...
    ' Каждый раз выделяем ПЕРВУЮ ячейку строки-образца и вставляем строку
    ' ниже неё через Selection.InsertRowsBelow — это работает даже
    ' при наличии вертикального merge выше (в шапке).
    If rowCount > 1 Then
        For i = 1 To rowCount - 1
...
' Структура: Rows 1-2 шапка (Row1 частично merge по gridSpan,
' Row 2 — подзаголовки), Row 3 — строка-образец для данных.
' ============================================================
...
' ============================================================
' Суммирует числовые значения в столбце Excel по буквенному адресу
' ============================================================
